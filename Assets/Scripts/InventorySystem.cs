...
public class InventorySystem : MonoBehaviour
{
    ...
    public void Consume(int id)
    {
        if(items[id].GetComponent<Item>().type == Item.ItemType.Consumables)
        {
            ...
            // Check if the item is a Cherry (Start name with "Cherry")
            if(items[id].name.StartsWith("Cherry") )
            {
                // Find the LifeCount component and add 2 lives
    ...
    {
        // Return a list of item IDs or names
        return items.Select(item => item.GetComponent<Item>().id).ToList();
    }

    public void SetItemList(List<string> itemIDs)
    {
        // Clear and repopulate inventory from IDs
        items.Clear();
        foreach (var id in itemIDs)
        {
            ItemDatabase.instance.GetItemById(id);
        }
    }
}
